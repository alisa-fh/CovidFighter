...
public class TileScript : MonoBehaviour
{
    ...
    public void Setup(Point gridPos, Vector3 worldPos, Transform parent)
    {
        ...
        else if ((gridPos.X == 6 && gridPos.Y == 4)||(gridPos.X == 14)) //water bridge + start points
        {
            IsEmpty = false; //can't place anything on it
            Walkable = true; //can walk on it
        }
        else
        {
            Walkable = true;
    ...
    {
        Walkable = false;
        if (AStar.GetPath(LevelManager.Instance.BridgeSpawn, LevelManager.Instance.StartPoints[0]) == null)
        {
        ...
        Walkable = false;
        //hides the range
        myTower.GetComponent<SpriteRenderer>().enabled = !myTower.GetComponent<SpriteRenderer>().enabled;
    }
