...
public class StoneTower : Tower
{
    ...
    public override string GetStats()
    {
        ...
        {
            return String.Format("<color=#36BAF3>{0}</color>{1} \nPercentage slowed: {2}% <color=#00ff00ff>+{3}%</color>", "<size=20><b>Stone</b></size>", base.GetStats(), SlowingFactor, NextUpgrade.SlowingFactor);
        }   

        //Returns the current upgrade
        return String.Format("<color=#36BAF3>{0}</color>{1} \nPercentage slowed: {2}%", "<size=20><b>Stone</b></size>", base.GetStats(), SlowingFactor);
    }
