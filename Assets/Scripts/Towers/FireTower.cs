...
public class FireTower : Tower
{
    ...
    public override string GetStats()
    {
        ...
        {
            return string.Format("<color=#ffa500ff>{0}</color>{1} \n <color=#00ff00ff></color>\nSpecial damage: {2} <color=#00ff00ff>+{3}</color>", "<size=30><b>Fire</b></size> ", base.GetStats(), TickDamage, NextUpgrade.SpecialDamage);
        }
 
        //Returns the current upgrade
        return string.Format("<color=#ffa500ff>{0}</color>{1} \nSpecial damage: {2}", "<size=30><b>Fire</b></size> ", base.GetStats(), TickDamage);
    }
