...
public class LevelManager : Singleton<LevelManager>
{
    ...
    [SerializeField]
    private GameObject[] villagePrefabs;
    private Point bridgeSpawn;
    public Point BridgeSpawn 
    {
        get 
        {
            return bridgeSpawn;
    ...
    private List<TileScript> stoneArray;
    public List<TileScript> StoneArray 
    {
        get 
        {
            return stoneArray;
    ...
    void Start()
    {
        stoneArray = new List<TileScript>();
    ...
    private void PlaceTile(string tileType, int x, int y, Vector3 worldStart)
    {
        ...
        if (x == 3 && y == 3 ) {
            Instantiate(villagePrefabs[0], Tiles[new Point(3,3)].GetComponent<TileScript>().WorldPosition, Quaternion.identity);

        }

        if (newTile.name.Contains("stone")) 
        {
            stoneArray.Add(newTile);
            if (y>=5)
            {
                ...
                int villageChance = UnityEngine.Random.Range(0,2);
                if (villageChance == 1) 
                {
                    int villageIndex = UnityEngine.Random.Range(1,6);
                    Point villageSpawn = new Point(x, y);
                    Instantiate(villagePrefabs[villageIndex], Tiles[villageSpawn].GetComponent<TileScript>().WorldPosition, Quaternion.identity);
                }
            }
        }
    ...
    private string[] ReadVillageText()
    {
        TextAsset bindData = Resources.Load("Village") as TextAsset;
        string data = bindData.text.Replace(Environment.NewLine, string.Empty);
        return data.Split('-');
    }
